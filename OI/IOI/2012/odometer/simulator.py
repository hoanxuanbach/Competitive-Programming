...
class Simulation:
    ...
    def __init__(self, grid_side=DEFAULT_GRID_SIDE,
        ...
        if grid is None:
            self.grid = [[0] * grid_side for i in range(grid_side)]
        else:
            self.grid = grid
        self.debug = debug
    ...
    def compile_runner(self):
        ...
        self.srcfile = os.path.join(self.compilation_dir, 'runner_withcode.c')
        ...
        with open(os.path.join(os.path.dirname(__file__), 'runner.c')) as fin:
            with open(self.srcfile, 'w') as fout:
                for line in fin:
                    ...
                    if 'MAX STEPS' in line:
                        fout.write('#define MAX_STEPS %d\n' % (self.max_steps or -1))
    ...
    def print_info(self):
        print("  Position is (%d, %d); direction is %s" % (self.posy, self.posx, dir_name[self.dir]))
        print("  Instruction pointer is %d" % (self.ip))
        print("  Dump of the grid")
        self.print_grid(self.grid_side)
        ...
        for y in range(side):
            for x in range(side):
                ...
                if x == self.posx and y == self.posy:
                    present = '*'
                print('%s%02d ' % (present, self.grid[x][y]), end=' ')
            print()
    ...
    def execute_step(self):
        ...
        if(jump_to == 55 and jumped): print(self.step_num)
        #print(self.posx,self.posy,self.dir)
...
def load_grid(fin, grid):
    for line in fin:
        ...
        try:
            posy, posx, num = list(map(int, line.split(' ')))
...
def dump_grid(fout, grid):
    for posx in range(len(grid)):
        for posy in range(len(grid[posx])):
            if grid[posx][posy] != 0:
