...
void solve(){
    ...
    for(int i=1;i<=N;i++){
        int x=i*i+1;
        vector<ull> F=factorize(x);
        map<int,int> mp;
        for(int d:F) mp[d]++;
        
        vector<pii> P;
        for(auto [k,d]:mp) P.push_back({k,d});

        function<void(int,int)> dfs = [&](int j,int T){
            if(j==(int)P.size()){
                int cur=i;
                if(cur>inf/(i+T)) return;
                cur*=(i+T);
                if(cur>inf/(i+x/T)) return;
                cur*=(i+x/T);
                X.push_back(cur);
                return;
            }
