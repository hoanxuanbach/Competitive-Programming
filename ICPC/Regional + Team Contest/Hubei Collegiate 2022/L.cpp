...
namespace Segtree{
    ...
    void update_pos(int l,int r,int id,int p){
        if(l==r){
            cnt[id]^=1;
            return;
        }
        pushdown(id);
        int mid=(l+r)>>1;
        if(p<=mid) update_pos(l,mid,id<<1,p);
        else update_pos(mid+1,r,id<<1|1,p);
        tree[id]=tree[id<<1]+tree[id<<1|1];
        cnt[id]=cnt[id<<1]+cnt[id<<1|1];
    }
...
void solve(){
    ...
    for(int i=1;i<=q;i++){
        int id,l,r,x;cin >> id;
        if(id<=2){cin >> x;Segtree::update_pos(1,n,1,x);}
        else if(id==3){
            cin >> l >> r >> x;
